...
def home(request):
    return render(request, 'A_Vastralaya_home/home.html')
    ...
    if request.method == 'POST':
        ...
        if  password1 == password2:
            if User.objects.filter(username=username).exists():
                ...
                return render(request, 'A_Vastralaya_home/register.html')        
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('A_Vastralaya-home')
        
        messages.error(request, "invalid username/password")
        return redirect('login')


    return render(request, 'A_Vastralaya_home/login.html')
...
def products(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'A_Vastralaya_home/products.html', context)
...
def productdetails(request,pk):
    ...
    return render(request,'A_Vastralaya_home/productdetails.html',context)
...
def cart(request):
    ...
    if len(product_ids) == 0:
        return render(request, 'A_Vastralaya_home/cart.html', context={})
    ...
    return render(request, 'A_Vastralaya_home/cart.html', context={'products': all_products, 'n_items': len(all_products)})
...
def checkout(request, pk):
    ...
    return render(request, 'A_Vastralaya_home/checkout.html', context = context)
...
def wallet(request):
    return render(request, 'A_Vastralaya_home/wallet.html', context={'wallet':  Wallet.objects.filter(user=request.user)[0].wallet})


def success(request):
    return render(request, 'A_Vastralaya_home/success.html')
...
def orders(request):
    ...
    if len(all_orders) == 0:
        context = {}
    return render(request, 'A_Vastralaya_home/orders.html', context)
...
def order_details(request, pk):
    ...
    if request.method == 'POST':
        ...
        if len(Rating.objects.filter(order=order)) == 0:
            ...
            f = False        
        return render(request, 'A_Vastralaya_home/order_details.html', context={'order': order, 'f': f, 'rating': rating})    
    
    return render(request, 'A_Vastralaya_home/order_details.html', context={'order': order, 'f': f, 'rating': rate_value})
...
def wishlist(request):
    ...
    if len(product_ids) == 0:
        return render(request, 'A_Vastralaya_home/wishlist.html', context={})
    
    all_products = []
    for prod in product_ids:
        all_products.append(Products.objects.get(id=prod.product.id))
    
    return render(request, 'A_Vastralaya_home/wishlist.html', context={'products': all_products, 'n_items': len(all_products)})
